...
public class InquiryFileUtils {
	...
	public List<AdminInquiryFileDto> parseFileInfo(int boardIdx, MultipartHttpServletRequest multiFiles) throws Exception {
...
//		매개변수로 받은 파일 정보에서 BoardFileDto 클래스에서 사용하는 정보만 추려내어서 리스트로 생성하여 저장할 변수
		List<AdminInquiryFileDto> fileList = new ArrayList<>();
//		날짜 사용 형식을 지정하는 클래스
...
//					System.nanoTime() : 1970년 1월 1일 00 시 00분 00초를 기준으로해서 현재시간까지 진행된 시간을 1/1000로 표현
					...
					AdminInquiryFileDto boardFile = new AdminInquiryFileDto();
...
//					boardFile.setNoticeBoardFileIdx(boardIdx);
					...
					boardFile.setInquiryIdx(boardIdx);
