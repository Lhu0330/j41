...
public class NoticeFileUtils {
	...
	public List<AdminNoticeFileDto> parseFileInfo(int boardIdx, MultipartHttpServletRequest multiFiles) throws Exception {
...
//		매개변수로 받은 파일 정보에서 BoardFileDto 클래스에서 사용하는 정보만 추려내어서 리스트로 생성하여 저장할 변수
		List<AdminNoticeFileDto> fileList = new ArrayList<>();
//		날짜 사용 형식을 지정하는 클래스
...
//					System.nanoTime() : 1970년 1월 1일 00 시 00분 00초를 기준으로해서 현재시간까지 진행된 시간을 1/1000로 표현
					...
					AdminNoticeFileDto boardFile = new AdminNoticeFileDto();
...
//					boardFile.setNoticeBoardFileIdx(boardIdx);
					...
					boardFile.setNoticeIdx(boardIdx);
...
//					MultipartFile 클래스의 getSize() 메서드의 반환타입이 long이기 때문에 SQL문에서 1메가가 넘어가는 경우 1,xxx로 표시되어 문자열로 인식이 되기 때문에 그대로 사용하지 못하고 문자열로 변환하여 사용
					boardFile.setNbfFileSize(Long.toString(mFile.getSize()));
//					현재 파일의 실제 파일명을 가져와서 저장
					boardFile.setNbfOriginalFileName(mFile.getOriginalFilename());
//					새로 생성된 파일명과 실제 디스크에 저장될 경로를 합하여 저장
					boardFile.setNbfStoredFilePath(path + "/" + newFileName);
